...
- Sitemap: removed the 4 noindex ad landing pages (intake, assistants, demand, case managers — paid ads only); dropped fake per-deploy `lastmod` on static pages
- Homepage footer: added Practice Areas links (PI, Employment, Workers' Comp, Checklist) so those pages are no longer orphaned (EN + ES keys)
